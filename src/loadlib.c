...
static const char *lua_path_sep = LUA_PATH_SEP;	// work around for compiler bug
...
static int readable (const char *filename) 
{
	...
	// wtof("loadlib.c:%s: filename=\"%s\"", __func__, filename);

	f = fopen(filename, "r");  /* try to open file */
	if (f == NULL) {
		// wtof("loadlib.c:%s: return 0", __func__);
		return 0;  /* open failed */
	}
	
	fclose(f);
	// wtof("loadlib.c:%s: return 1", __func__);
	return 1;
}
...
static const char *getnextfilename (char **path, char *end) 
{
	...
	*path = sep;  /* will start next search from here */
	...
	// wtodumpf(name, strlen(name), "loadlib.c:%s: name", __func__);
	return name;
}
